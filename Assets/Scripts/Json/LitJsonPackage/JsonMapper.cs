...
namespace LitJson
{
    ...
    public class JsonMapper
    {
        ...
        private static void AddObjectMetadata(Type type)
        {
            ...
            foreach (PropertyInfo p_info in type.GetProperties())
            {
                if (p_info.Name == "Item")
                {
                    ...
                    if (parameters.Length == 1)
                    {
                        data.KeyType = parameters[0].ParameterType;
                        data.ElementType = p_info.PropertyType;
                    }
            ...
            else if (reader.Token == JsonToken.ObjectStart)
            {
                ...
                while (true)
                {
                    ...
                    else
                    {
                        ...
                        object key = ConvertDictionaryKey(property, t_data.KeyType);
        ...
        //新增
        private static object ConvertDictionaryKey(string property, Type keyType)
        {
            ...
            if (keyType.IsEnum)
            {
                if (int.TryParse(property, out int enumValue))
                {
                    return Enum.ToObject(keyType, enumValue);
                }

                return Enum.Parse(keyType, property, false); //区分大小写
            }

            return Convert.ChangeType(property, keyType);
        }
        //
